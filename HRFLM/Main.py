...
def logisticRegression():
    ...
    global X, Y, X_train, X_test, y_train, y_test
    text.delete('1.0', END)
    cls = LogisticRegression(penalty='l2', dual=False, tol=0.002, C=2.0,solver='liblinear')
    cls.fit(X_train, y_train)
    text.insert(END,"Prediction Results\n\n") 
    prediction_data = prediction(X_test, cls) 
...
def HRFLM():
    ...
    global hrflm_acc
    global X, Y, X_train, X_test, y_train, y_test
    lrc = LogisticRegression(max_iter=100, multi_class='auto',class_weight='balanced',solver='liblinear')
